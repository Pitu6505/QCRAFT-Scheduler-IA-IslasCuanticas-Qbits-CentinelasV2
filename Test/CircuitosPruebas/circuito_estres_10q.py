from qiskit import QuantumRegister, ClassicalRegister, QuantumCircuit
import numpy as np

qreg_q = QuantumRegister(10, 'q')
creg_c = ClassicalRegister(10, 'c')
circuit = QuantumCircuit(qreg_q, creg_c)

# ================================
# CAPA 1: Preparación Densa (Rotaciones)
# ================================
circuit.rx(np.pi/2, qreg_q[0])
circuit.rz(np.pi/4, qreg_q[0])
circuit.rx(np.pi/2, qreg_q[1])
circuit.rz(np.pi/4, qreg_q[1])
circuit.rx(np.pi/2, qreg_q[2])
circuit.rz(np.pi/4, qreg_q[2])
circuit.rx(np.pi/2, qreg_q[3])
circuit.rz(np.pi/4, qreg_q[3])
circuit.rx(np.pi/2, qreg_q[4])
circuit.rz(np.pi/4, qreg_q[4])
circuit.rx(np.pi/2, qreg_q[5])
circuit.rz(np.pi/4, qreg_q[5])
circuit.rx(np.pi/2, qreg_q[6])
circuit.rz(np.pi/4, qreg_q[6])
circuit.rx(np.pi/2, qreg_q[7])
circuit.rz(np.pi/4, qreg_q[7])
circuit.rx(np.pi/2, qreg_q[8])
circuit.rz(np.pi/4, qreg_q[8])
circuit.rx(np.pi/2, qreg_q[9])
circuit.rz(np.pi/4, qreg_q[9])

# ================================
# CAPA 2: Entrelazamiento Masivo (Crosstalk)
# ================================
circuit.cx(qreg_q[0], qreg_q[1])
circuit.cx(qreg_q[1], qreg_q[2])
circuit.cx(qreg_q[2], qreg_q[3])
circuit.cx(qreg_q[3], qreg_q[4])
circuit.cx(qreg_q[4], qreg_q[5])
circuit.cx(qreg_q[5], qreg_q[6])
circuit.cx(qreg_q[6], qreg_q[7])
circuit.cx(qreg_q[7], qreg_q[8])
circuit.cx(qreg_q[8], qreg_q[9])
circuit.cx(qreg_q[9], qreg_q[0])
circuit.cx(qreg_q[9], qreg_q[8])
circuit.cx(qreg_q[8], qreg_q[7])
circuit.cx(qreg_q[7], qreg_q[6])
circuit.cx(qreg_q[6], qreg_q[5])
circuit.cx(qreg_q[5], qreg_q[4])
circuit.cx(qreg_q[4], qreg_q[3])
circuit.cx(qreg_q[3], qreg_q[2])
circuit.cx(qreg_q[2], qreg_q[1])
circuit.cx(qreg_q[1], qreg_q[0])

# ================================
# CAPA 3: SWAPs y Toffolis (Fatiga Térmica T1)
# ================================
circuit.swap(qreg_q[0], qreg_q[2])
circuit.swap(qreg_q[2], qreg_q[4])
circuit.swap(qreg_q[4], qreg_q[6])
circuit.swap(qreg_q[6], qreg_q[8])
circuit.ccx(qreg_q[0], qreg_q[1], qreg_q[2])
circuit.ccx(qreg_q[3], qreg_q[4], qreg_q[5])
circuit.ccx(qreg_q[6], qreg_q[7], qreg_q[8])

# ================================
# CAPA 4: Rotaciones de Colisión Final
# ================================
circuit.u(np.pi/2, np.pi/4, np.pi/8, qreg_q[0])
circuit.u(np.pi/2, np.pi/4, np.pi/8, qreg_q[1])
circuit.u(np.pi/2, np.pi/4, np.pi/8, qreg_q[2])
circuit.u(np.pi/2, np.pi/4, np.pi/8, qreg_q[3])
circuit.u(np.pi/2, np.pi/4, np.pi/8, qreg_q[4])
circuit.u(np.pi/2, np.pi/4, np.pi/8, qreg_q[5])
circuit.u(np.pi/2, np.pi/4, np.pi/8, qreg_q[6])
circuit.u(np.pi/2, np.pi/4, np.pi/8, qreg_q[7])
circuit.u(np.pi/2, np.pi/4, np.pi/8, qreg_q[8])
circuit.u(np.pi/2, np.pi/4, np.pi/8, qreg_q[9])

# ================================
# Barrera y Mediciones
# ================================
circuit.barrier()
circuit.measure(qreg_q[0], creg_c[0])
circuit.measure(qreg_q[1], creg_c[1])
circuit.measure(qreg_q[2], creg_c[2])
circuit.measure(qreg_q[3], creg_c[3])
circuit.measure(qreg_q[4], creg_c[4])
circuit.measure(qreg_q[5], creg_c[5])
circuit.measure(qreg_q[6], creg_c[6])
circuit.measure(qreg_q[7], creg_c[7])
circuit.measure(qreg_q[8], creg_c[8])
circuit.measure(qreg_q[9], creg_c[9])